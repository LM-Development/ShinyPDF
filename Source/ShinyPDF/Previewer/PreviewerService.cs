...
async Task<Version> GetPreviewerVersion()
        ...
        private void StartPreviewer()
        {
            ...
            {
                throw new Exception("Cannot start the ShinyPDF Previewer tool. " +
                                    "Please install it by executing in terminal the following command: 'dotnet tool install --global ShinyPDF.Previewer'.");
            }
        }
        ...
        private void CheckVersionCompatibility(Version version)
        {
            ...
            throw new Exception($"Previewer version is not compatible. Possible solutions: " +
                                $"1) Update the ShinyPDF library to newer version. " +
                                $"2) Update the ShinyPDF previewer tool using the following command: 'dotnet tool update --global ShinyPDF.Previewer --version {RequiredPreviewerVersionMajor}.{RequiredPreviewerVersionMinor}'");
        }
        ...
        private async Task WaitForConnection()
        {
            ...
            while (true)
            {
                ...
                if (cancellationToken.IsCancellationRequested)
                    throw new Exception($"Cannot connect to the ShinyPDF Previewer tool. Please make sure that your Operating System does not block HTTP connections on port {Port}.");
