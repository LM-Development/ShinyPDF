...
namespace ShinyPDF.Drawing
{
    public static class FontManager
    {
        ...
        internal static SKTypeface? MatchFamily(string? fontFamily) => MatchFamily(fontFamily, SKFontStyle.Normal);
        internal static SKTypeface? MatchFamily(string? fontFamily, SKFontStyle fontStyle)
        {
            if (fontFamily == null)
                return null;

            if (StyleSets.TryGetValue(fontFamily, out var fontStyleSet))
            {
                var typeface = fontStyleSet.Match(fontStyle);
                if (typeface != null)
                    return typeface;
            }

            return SKFontManager.Default.MatchFamily(fontFamily, fontStyle);
        }

        public static string[] GetRegisteredFontFamilies()
        {
            var defaultFontFamilies = SKFontManager.Default.GetFontFamilies();
            var customFontFamilies = StyleSets.Keys;

            return defaultFontFamilies.Concat(customFontFamilies).ToArray();
        }
        ...
        {
            var filePathFormat = "ShinyPDF.Resources.DefaultFonts.{0}";
            var fonts = new[]
            {
                "Lato",
                "Courier",
            ...
            var fontFileNames = fonts.SelectMany(font => Assembly.GetExecutingAssembly().GetManifestResourceNames()
                .Where(resourceName => resourceName.StartsWith(string.Format(filePathFormat, font), StringComparison.OrdinalIgnoreCase) && resourceName.EndsWith(".ttf", StringComparison.OrdinalIgnoreCase)))
                .ToList();
            ...
            foreach (var fileName in fontFileNames)
            {
                ...
                if (stream == null)
                    throw new InvalidOperationException($"Cannot load default font file from an embedded resource. Please make sure that the resource is available: {fileName}");
                RegisterFont(stream);
            }
        }
        ...
        private static SKTypeface GetTypeface(TextStyle style)
        {
            ...
            var availableFontNames = string.Join(", ", SKFontManager.Default.GetFontFamilies().Concat(StyleSets.Keys));
            ...
            throw new ArgumentException(
                ...
                $"2) load a font file specifically for ShinyPDF usage via the ShinyPDF.Drawing.FontManager.RegisterFontType(Stream fileContentStream) static method. " +
