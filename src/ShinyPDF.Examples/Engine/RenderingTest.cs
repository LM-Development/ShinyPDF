...
using ShinyPDF.Examples.Helpers;
        ...
        private void Render(IDocument document)
        {
            if (ResultType == RenderingTestResult.Images)
            {
                ...
                if (ShowResult && ShowingResultsEnabled)
                    ShowingResultsEnabled = FileOpeningHelper.TryOpen(fileNameSchema(0));
            }
            ...
            if (ResultType == RenderingTestResult.Pdf)
            {
                ...
                if (ShowResult && ShowingResultsEnabled)
                    ShowingResultsEnabled = FileOpeningHelper.TryOpen(fileName);
            }
        }
